...
# You will do that by defining the following three functions:
...
def update(tuples, key, val):
    """Return a new tuple of (key, val), dropping old value of key and adding new."""
    d = dict(tuples)
    d[key] = val
...
def test_parking():
    ...
    assert same_state(grid((('*', locs(25, 2)),
                            ('B', locs(19, 3, N)),
                            ('P', locs(36, 3)),
                            ('O', locs(45, 2, N)),
                            ('Y', locs(49, 3)))),
...
def legal_step(path):
    "A legal step has an action that leads to a state that are valid successors."
