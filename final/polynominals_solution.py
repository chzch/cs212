Solution:
I wanted to make calls like p1(0) reasonably efficient. Some choices I considered: (1) Passing the __name__ to eval: that would work, but would do unneeded work. (2) Looping through the coefs, summing terms: faster, but not fast enough. (3) Compile the arithmetic formula just once, then run the function: that's it!
And to make things a little faster, I use Horner's rule, which stops me from having to evaluate exponents multiple times; I just add and multiply. But I use the familiar formula for the name of the polynomial.
I arrange so that my top-level function poly calls polynomial, which is memoized. That way, the same coefficients will yield identically the same function (unless someone resets the memo cache).
