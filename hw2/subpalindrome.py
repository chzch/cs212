...
def longest_subpalindrome_slice(text):
    ...
    length, plist = len(text), [] 
    space = 2 * length + 1

    for x in xrange(space):
        i = x / 2
        j = i + x % 2
        while i > 0 and j < length and text[i - 1].upper() == text[j].upper():
            i -= 1
            j += 1
        plist.append( (i, j) )

    return  max(plist, key=lambda t: t[1]-t[0])
