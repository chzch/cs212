...
def pig_actions_d(state):
    ...
    if isinstance(double, str): return ["accept", "decline"]
    if double == 1: return ["roll", "hold", "double"] if pending else ["roll", "double"]
    if double > 1: return ["roll", "hold"] if pending else ["roll"]
   
def strategy_d(state):
    (p, me, you, pending, double) = state
    return ('double' if (double == 1 and pending >= 20 and me + pending >= goal) else
            'hold' if (double == 2 and pending >= 20 and me + pending >= goal) else
            'roll')
